        ...
        for hop_count in data:
            if run_count <= int(hop_count):
                ...
                for item in data[hop_count]:
                    ...
                    retrieved = retrieve_doc_titles(item["not_supported_counterpart"][f"claim_{run_count}"], embedder.task, vector_db, embedder, k=100)
                    item["not_supported_counterpart"][f"retrieved_{run_count}"] = retrieved
                ...
                for item in data[hop_count]:
                    ...
                    context = ""
                    for i, sub_question in enumerate(item["not_supported_counterpart"][f"sub_questions_{run_count}"]):
                        answer = item["not_supported_counterpart"][f"question_answering_output_{run_count}"][i]
                        context += f"{sub_question}\n{answer}\n"
                    context = context.rstrip("\n")
                    claims_batch_not_supported.append({"claim": item["not_supported_counterpart"][f"claim_{run_count}"], "context": context})
                    

                for i in range(0, len(claims_batch), 5):
                    sub_list = claims_batch[i:i+5]
                    refinement_batch_output = base_refinement_chain.batch(sub_list)
                    for item_index, item in enumerate(sub_list):
                        data[hop_count][i+item_index][f"claim_{run_count+1}"] = refinement_batch_output[item_index]
                        

                    sub_list = claims_batch_not_supported[i:i+5]
                    refinement_batch_output = base_refinement_chain.batch(sub_list)
                    for item_index, item in enumerate(sub_list):
                        data[hop_count][i+item_index]["not_supported_counterpart"][f"claim_{run_count+1}"] = refinement_batch_output[item_index]




        save_obj(data, "data/iterative_qualitative_analysis_not_supported_question_answering_80.json")
