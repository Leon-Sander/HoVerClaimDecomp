...
for i, template in enumerate([decompose_9shot_instruct, decompose_entity_based, decompose_without_redundancy]):
    #data = load_obj("data/iteration_full_data.json")
    #data = load_obj("data/iterative_TFIDF_full_data_output.json")
    data = load_obj("data/iterative_decomp_FULL_DATASET.json")
    ...
    try:
        for run_count in tqdm(range(4), desc='Run Count'):
            ...
            save_obj(data, f"data/iterative_decomp_FULL_DATASET_{i}.json")


    except Exception as e:
        # print traceback
        save_obj(data, "data/iterative_decomp_FULL_DATASET_ZWISCHENERGEBNIS.json")
        print(e)
