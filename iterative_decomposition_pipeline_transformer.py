...
#for i, template in enumerate([decompose_entity_based, decompose_without_redundancy]):
#data = load_obj("data/iteration_full_data.json")
#data = load_obj("data/iterative_TFIDF_full_data_output.json")
data = load_obj("data/iterative_FULL_DATASET_subquestion_ABLATION.json")
    ...
    for hop_count in data:
        if run_count <= int(hop_count):
            for key in data[hop_count]:
                ...
                if not all_claims:
                    print(f"No decomposition to generate for {run_count}, {hop_count}, {key}")
                    continue
...
try:
    ...
    save_obj(data, f"data/iterative_decomp_FULL_DATASET_ABLATION.json")
    del llm1
    del llm2
    torch.cuda.empty_cache()
    gc.collect()
    ...
    # print traceback
    save_obj(data, f"data/iterative_decomp_FULL_DATASET_ABLATION_ZWISCHENERGEBNIS.json")
    print(e)
