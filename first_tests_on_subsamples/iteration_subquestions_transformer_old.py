...
def claim_refinement(run_count):
    for hop_count in data:
        if run_count <= int(hop_count) - 1:
            for key in data[hop_count]:
                all_claims = [item[f"claim_{run_count}"] for item in data[hop_count][key]]
                all_top_sentences = [item[f"top_sentences_{run_count}"] for item in data[hop_count][key]]
                for j in tqdm(range(0, len(all_claims), 8), desc=f'Refining Base Claims {run_count}, {hop_count}, {key}'):
                    base_claims_context = [(all_claims[j + k], all_top_sentences[j + k]) for k in range(8) if j + k < len(all_claims)]
