...
def process_claims(llm, batch_claims):
    sub_questions_batch = llm.generate_subquestions(batch_claims)
    return sub_questions_batch


def run_in_parallel(all_claims):
    mid_point = len(all_claims) // 2
    first_half = all_claims[:mid_point]
    second_half = all_claims[mid_point:]

    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            executor.submit(process_claims, llm1, first_half),
            executor.submit(process_claims, llm2, second_half)
        ]
...
try:
            ...
            if run_count <= int(hop_count) - 1:
                for key in data[hop_count]:
                    ...
                    for i in tqdm(range(0, len(all_claims), 50), desc='Processing Claims in Batches of 50'):
                        ...
                        sub_questions_batch = run_in_parallel(batch_claims)
                    ...
                    for j in tqdm(range(0, len(all_claims), 10), desc='Refining Base Claims'):
                        ...
                        enhanced_claims = run_in_parallel(base_claims_context)
    ...
    save_obj(data, "data/iterative_FULL_DATASET_with_questions_60_no_filter_ZWISCHENERGEBNIS.json")
    save_obj(data2, "data/iterative_FULL_DATASET_sentences_ZWISCHENERGEBNIS.json")